...
import optax
from model_util import (FlaxBaseModelOutput,
...
def test_bert_layer():
    ...
    trans_state=TrainState.create(apply_fn=model.apply,params=params,tx=optimizer)
    ...
    def train_step(trans_state, batch):

        def loss_func(params):
            rngs = {"dropout": batch["rng"]}
            out = trans_state.apply_fn(params,
                              batch["hidden_states"],
                              batch["attention_mask"],
                              rngs=rngs)[0]
            return jnp.mean((out - batch["label"])**2)

        grads = jax.grad(loss_func)(trans_state.params)
        trans_state = trans_state.apply_gradients(grads=grads)
        return trans_state
    ...
    jaxpr = jax.make_jaxpr(train_step)(trans_state, {
        "hidden_states": hidden_states,
        "attention_mask": attention_mask,
        "label": label,
        "rng": rngkey
...
def test_bert_mlm():
    ...
    def train_step(trans_state, batch):
        ...
        def loss_func(params):
            rngs = {"dropout": batch["rng"]}
            logits = trans_state.apply_fn(params,
                                batch["input_ids"],
                                batch["attention_mask"],
